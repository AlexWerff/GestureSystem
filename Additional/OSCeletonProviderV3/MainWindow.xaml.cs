...
namespace Microsoft.Samples.Kinect.FaceBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private List<ulong> bodyTrackingIDs;
        ...
        public MainWindow()
        {
            ...
            this.bodyTrackingIDs = new List<ulong>();
        ...
        {
            foreach (ulong b in this.bodyTrackingIDs)
            {
                osceleton.LostUser(b);
            }
        ...
        private void Sensor_IsAvailableChanged(object sender, IsAvailableChangedEventArgs e)
        {
            if (kinectSensor != null)
            {
                ...
                {
                    foreach (ulong b in bodyTrackingIDs)
                    {
                        osceleton.LostUser(b);
                    }
                }
        ...
        private void Reader_FrameArrived(object sender, BodyFrameArrivedEventArgs e)
        {
            ...
            using (BodyFrame bodyFrame = e.FrameReference.AcquireFrame())
            {
                if (bodyFrame != null)
                {
                    ...
                    foreach (ulong id in this.bodyTrackingIDs.ToArray())     // Known bodies
                    {
                        Boolean existing = false;

                        for (int i = 0; i < this.bodyCount; i++)
                        {
                            if (id == this.bodies[i].TrackingId)
                            {
                                existing = true;
                            }
                        }

                        if (!existing)  // Body "id" is gone
                        {
                            bodyTrackingIDs.Remove(id);
                            osceleton.LostUser(id);
                        }
                    }

                    for (int i = 0; i < this.bodyCount; i++)
                    {
                        ulong id = this.bodies[i].TrackingId;

                        this.osceleton.EnqueueBody(0, (int)id, this.bodies[i]);
                        if (!this.bodyTrackingIDs.Contains(id))
                        {
                            // This body is new in the frame
                            this.bodyTrackingIDs.Add(id);
                            this.osceleton.NewUser(id);
                        }
                    }
                    dataReceived = true;
                }
        ...
        private static RecognizerInfo TryGetKinectRecognizer()
        {
            ...
            foreach (RecognizerInfo recognizer in recognizers)
            {
                ...
                if ("True".Equals(value, StringComparison.OrdinalIgnoreCase) &&
                    "de-de".Equals(recognizer.Culture.Name, StringComparison.OrdinalIgnoreCase))
                {
                    return recognizer;
                }
            }
        ...
        private void StartSpeechRecognizer()
        {
            ...
            if (null != ri)
            {
                ...
                this.speechEngine.SpeechDetected += (obj,e) =>
                {
                    Console.WriteLine(e.AudioPosition);
                };
                this.speechEngine.SpeechRecognitionRejected += (obj, e) =>
                {
                    Console.WriteLine(e.Result);
                };
                this.speechEngine.SpeechHypothesized += (obj, e) =>
                {
                    Console.WriteLine(e.Result);
                };
            ...
            {
                this.helpbox.Text = "No Speech Recognizer found (correct language? x86 vs. x64?).";
            }
        ...
        private void initOSCeleton(int port=7110,String host="127.0.0.1")
        {
            initFaceDB();
            this.osceleton.Initialise(port,host);
        ...
        {
            helpbox.Text = e.Result.Text; 
            osceleton.SpeechRecognized(e.Result.Text,e.Result.Semantics.Value.ToString(), e.Result.Confidence);
        }

        private void ButtonSave_Click(object sender, RoutedEventArgs e)
        {
            initOSCeleton(int.Parse(TextBoxPort.Text), TextBoxIP.Text);
        }
    }
}
